...
AStar::AStar() : _walls(nullptr),
				 _allowedDirectionsCount(0),
				 _mazeWidth(0),
				 _mazeHeight(0)
{
...
int32_t AStar::init(const int32_t              mazeWidth,
					...
					const std::vector<Point> * walls)
{
	...
	_allowedDirectionsCount = (isDiagonalMovementEnabled) ? 8 : 4;
...
std::vector<Point> AStar::findPath(const Point & source, const Point & target)
{
	...
	{
		fprintf(stderr, "Error, source or target are out of bounds\n");
		return path;
	}
    ...
    while (!openSet.empty())
    {
        ...
        for (uint32_t i = 0; i < _allowedDirectionsCount; ++i)
        {
            ...
            int32_t totalCost = current->G + ((i < 4) ? 10 : 14);
...
bool AStar::detectCollision(const Point & position)
{
    if (position.x < 0 || position.x >= _mazeWidth  ||
        position.y < 0 || position.y >= _mazeHeight ||
        std::find((*_walls).begin(), (*_walls).end(), position) !=
        														(*_walls).end())
    {
        return true;
    }
